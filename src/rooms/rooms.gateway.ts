...
export class RoomsGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleDisconnect(client: Socket) {
    ...
    if (roomId && userId) {
      // Cleanup 1: Remove user from DB room list
      await this.roomsService.removeUserFromRoom(roomId, userId);

      // Cleanup 2: Remove user from Redis set
  ...
  async handleJoinRoom(
    ...
    try {
      // Keep user metadata on socket object
      client.data.userId = data.userId;
      client.data.username = data.username;

      // Step 1: Add user to room in MongoDB
      ...
      // Step 2: Add user to Redis active user set
      await this.locationCacheService.addRoomUser(data.roomId, data.userId);

      // ⭐ Step 3: CREATE INITIAL LOCATION FOR USER (CRITICAL FIX)
      await this.roomsService.updateUserLocation({
        userId: data.userId,
        username: data.username,
        roomId: data.roomId,
        x: 1200, // spawn location
        y: 50,
      });

      // Step 4: Add socket to room
      client.join(data.roomId);
      this.userRooms.set(client.id, data.roomId);

      // Step 5: Fetch current room state
      const users = await this.roomsService.getRoomUsers(data.roomId);
      const locations = await this.roomsService.getActiveLocationsInRoom(
        data.roomId,
      );

      // Step 6: Send existing players + positions to current user
